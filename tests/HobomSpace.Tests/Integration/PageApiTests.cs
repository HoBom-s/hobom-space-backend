...
public class PageApiTests(IntegrationTestFixture fixture)
{
    ...
    public async Task CreatePage_ReturnsCreated()
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<ApiResponse<PageResponse>>();
        body!.Items!.Title.Should().Be("Test Page");
    }
    ...
    public async Task UpdatePage_ReturnsOk()
    {
        ...
        var created = await createResponse.Content.ReadFromJsonAsync<ApiResponse<PageResponse>>();

        var response = await _client.PutAsJsonAsync(
            $"/api/v1/spaces/PGUP/pages/{created!.Items!.Id}",
            new UpdatePageRequest("Updated", "New Content", null));

        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var body = await response.Content.ReadFromJsonAsync<ApiResponse<PageResponse>>();
        body!.Items!.Title.Should().Be("Updated");
    }

    [Fact]
    public async Task DeletePage_ReturnsOk()
    {
        ...
        var response = await _client.DeleteAsync($"/api/v1/spaces/PGDL/pages/{created!.Items!.Id}");
